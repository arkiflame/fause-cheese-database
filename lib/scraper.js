...
async function scrapeCheeseFromUrl(url) {
  ...
  // Try LLM first if API key is set; otherwise use heuristics
  const pageText = extractPageTextForLlm($);
  const llmDesc = await generateCheeseDescription(pageText, {
    name: result.name,
    origin: result.origin,
    milk: result.milk,
  });

  result.description = llmDesc || buildThreeSentenceDescription(
    $,
    result.origin,
    result.name,
    result.milk
